...
spawnDefensiveHarrier( lifeId, owner, pathStart, pathGoal )
{
	...
	harrier thread harrierLeaveOnOwnerSpawn( owner );
...
harrierLeaveOnOwnerSpawn( owner )
{
	self endon( "death" );
...
getNewPoint( pos, targ )
{
	...
	if ( !isDefined( targ ) )
	{
		...
		foreach( spawnpoint in level.teamSpawnPoints[self.team] )
		{
			if ( coinToss() )
			        continue;
		...
		{
			gotoPoint = AveragePoint( teamPoints );
		...
		{
			gotoPoint = AveragePoint( spawnPoints );
			
			pointX = gotoPoint[0];
			pointY = gotoPoint[1];
		}
		else if( enemyPoints.size > 0 )
		{
...
harrierGetTargets()
{
	...
	for ( ;; )
	{
		...
		{
			foreach( helis in level.helis )
			{
				if( !isDefined( helis ) )
					continue;
				
				if ( (level.teamBased && helis.team != self.team) || (!level.teamBased && helis.owner != self.owner) )
				{
					self thread engageVehicle( helis );
					return;
				}
			}
		}
		...
		if ( isDefined( level.littlebird ) )
		{
			foreach( littlebird in level.littlebird )
			{
				...
				{
					self thread engageVehicle( littlebird );
					return;
				}
			}
		}
		...
		{	
				self thread engageVehicle( level.ac130.planeModel );
				return;
...
isTarget( potentialTarget )
{
	...
	if ( distance( potentialTarget.origin, self.origin ) > 4096 )
		return false;
	...
	//if ( potentialTarget _hasPerk( "specialty_armorvest" ) )
		//return false;
...
getBestTarget( targets )
{
	...
	foreach ( targ in targets )
	{
		...
		{
			if ( isSubStr( weapon, "at4" ) || isSubStr( weapon, "stinger" ) || isSubStr( weapon, "jav" ) || isSubStr( weapon, "rpg" ) )
				{ 
					angle -= 40;
					self thread evasiveManuverOne();
				}
		}
		...
		if ( !bulletTracePassed(targ getTagOrigin( "j_head" ), targ getTagOrigin( "j_head" )+(0,0,255), false, targ )  )
			angle += 40;

		//with painkiller perk
		if ( targ _hasPerk( "specialty_armorvest" ) )
			angle += 40;

		//they have cold blooded
		if ( targ _hasPerk( "specialty_coldblooded" ) )
			angle += 40;

		if( !isDefined(targ.pers["isBot"]) )
			angle += 40;
