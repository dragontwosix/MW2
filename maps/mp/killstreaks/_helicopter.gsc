...
init()
{
	...
	level.heli_missile_rof 	= 4;	// missile rate of fire, one every this many seconds per target, could fire two at the same time to different targets
	...
	level.heli_turretReloadTime = 1.0;	// mini-gun reload time
	level.heli_turretClipSize 	= 50;	// mini-gun clip size, rounds before reload
	...
	level.heli_missile_target_cone 		= 0.77;		// dot product of vector target to helicopter forward, 0.5 is in 90 range, bigger the number, smaller the cone
...
startHelicopter( lifeId, heliType )
{
	...
	switch ( heliType )
	{
		...
		case "minigun":
			...
			//start this chopper one node closer :)
...
weaponLockThink( chopper )
{
	...
	for ( ;; )
	{
		...
		foreach ( player in level.players )
		{
			...
			if ( player _hasPerk( "specialty_armorvest" ) )
				continue;
		...
		*/
		...
		if ( targetsInReticle.size != 0 )
		{
			...
			else
			{
				...
				self SetLookAtEnt ( sortedTargets[0] );
				heliDialog( "tracking" );
			}
		}
		else
...
{
	heliOrigin = startnode.origin+(0,0,200);
	...
	{
		case "minigun":
			chopper thread maps\mp\killstreaks\_emp::ShootLBJavi(owner);
			chopper thread heli_targeting();
			chopper heli_fly_simple_path( startNode );
			chopper thread heli_leave_on_timeout( 50.0 );
			if ( attackAreas.size )
				chopper thread heli_fly_well( attackAreas );
			else
				chopper thread maps\mp\killstreaks\_emp::LBHuntEnemies();
			break;
		case "flares":
...
heli_targeting()
{
	...
	{		
		...
		if ( targets.size )
		{
			...
			{
			self SetLookAtEnt ( self.secondaryTarget );
			} else {
		...
		if ( isDefined( level.littlebird ) )
		{
			foreach( littlebird in level.littlebird )
			{
				...
				{
					self notify( "secondary acquired" );
					self.secondaryTarget = littlebird;
				}
			}
		}
		...
		{	
				self notify( "secondary acquired" );
				self.secondaryTarget = level.ac130.planeModel;
...
attack_primary()
{
	...
	while ( 1 )
	{
		...
		for ( i = 0; i < level.heli_turretClipSize; i++ )
		{
			...
			targetPos = ( (xOffset*convergenceMod)+randomFloatRange( -2, 2 ),(yOffset*convergenceMod)+randomFloatRange( -2, 2 ),40+randomFloatRange( -2, 2 ) );
...
heli_fly_loop_path( startNode )
{
	...
	//some maps helis fly too high, so lets have them fly lower   0=normal  1=lower 2=lowest
	flyLowZ = 0;
	...
	while ( isDefined( currentNode.target ) )
	{
		...
		if ( isDefined( nextNode.script_delay ) && isDefined( self.primaryTarget ) && !self heli_is_threatened() )
		{
			if(flyLowZ==2)
			{
				self setVehGoalPos( nextNode.origin - (randomInt(200),randomInt(200),randomIntRange(600,1000)), true );
			}
			else if(flyLowZ==1)
			{
				self setVehGoalPos( nextNode.origin - (randomInt(200),randomInt(200),randomIntRange(100,300)), true );
			}
			else
			{
				self setVehGoalPos( nextNode.origin + (randomInt(200),randomInt(200),randomIntRange(100,200)), true );
			}
			self waittill( "near_goal" );
		...
		else
		{
			if(flyLowZ==2)
			{
				self setVehGoalPos( nextNode.origin - (randomInt(200),randomInt(200),randomIntRange(600,1000)), false );
			}
			else if(flyLowZ==1)
			{
				self setVehGoalPos( nextNode.origin - (randomInt(200),randomInt(200),randomIntRange(100,300)), false );
			}
			else
			{
				self setVehGoalPos( nextNode.origin + (randomInt(200),randomInt(200),randomIntRange(100,200)), false );
			}
			self waittill( "near_goal" );
...
heli_leave()
{
	...
	//added code to make sure the helicopter can SEE the leave node (if not raise it's altitude so it doesn't fly through buildings)
	//this is most noticable with the chopper gunner when it uses attack nodes
	pathGoal = self.origin;

	for( i = 1 ; i <= 100 ; i++ )
	{
		leaveTrace = BulletTrace(self.origin, leaveNode.origin-(0,0,50), false, self );
