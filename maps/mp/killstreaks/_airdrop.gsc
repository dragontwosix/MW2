...
doFlyBy( owner, dropSite, dropYaw, dropType, heightAdjustment )
{
	...
	{
		chopper thread maps\mp\killstreaks\_harrier::harrierGetTargets();
		chopper thread maps\mp\killstreaks\_harrier::evasiveManuverOne();
...
heliSetup( owner, pathStart, pathGoal )
{
	...
	mgTurret = spawnTurret( "misc_turret", chopper.origin, "pavelow_minigun_mp" );
	mgTurret.lifeId = self.lifeid;
	mgTurret linkTo( chopper, "tag_minigun_attach_left", ( 0,0,0 ), ( 0,0,0) );
	mgTurret setModel( "weapon_minigun" );
	mgTurret.owner = owner;
 	mgTurret.team = owner.team;
 	mgTurret makeTurretInoperable();
 	mgTurret.pers["team"] = owner.team;
 	mgTurret.killCamEnt = chopper;
 	chopper.mgTurretLeft = mgTurret; 
 	chopper.mgTurretLeft SetDefaultDropPitch( 8 );
	chopper.mgTurretLeft SetRightArc(30);
   	chopper.mgTurretLeft SetLeftArc(30);
   	chopper.mgTurretLeft SetBottomArc(75);

	mgTurret = spawnTurret( "misc_turret", chopper.origin, "pavelow_minigun_mp" );
	mgTurret.lifeId = self.lifeid;
	mgTurret linkTo( chopper, "tag_minigun_attach_right", ( 0,0,0 ), ( 0,0,0) );
	mgTurret setModel( "weapon_minigun" );
	mgTurret.owner = owner;
 	mgTurret.team = owner.team;
 	mgTurret makeTurretInoperable();
 	...
 	chopper.mgTurretRight SetDefaultDropPitch( 8 );
	chopper.mgTurretRight SetRightArc(30);
   	chopper.mgTurretRight SetLeftArc(30);
   	chopper.mgTurretRight SetBottomArc(75);

	if ( level.teamBased )
	{
		chopper.mgTurretLeft setTurretTeam( owner.team );
		chopper.mgTurretRight setTurretTeam( owner.team );
	}

 	chopper.mgTurretLeft setMode( "auto_nonai" );
 	chopper.mgTurretRight setMode( "auto_nonai" );
 	
	chopper.mgTurretLeft SetSentryOwner( self.owner );
	chopper.mgTurretRight SetSentryOwner( self.owner );
	
	chopper.mgTurretLeft SetTurretMinimapVisible( false );
	chopper.mgTurretRight SetTurretMinimapVisible( false );

	chopper.mgTurretLeft thread maps\mp\killstreaks\_helicopter::sentry_attackTargets();
	chopper.mgTurretRight thread maps\mp\killstreaks\_helicopter::sentry_attackTargets();
